...
fil <- tempfile("simulation_dataset", fileext = ".rds")
## save a single object to file
saveRDS(FinalData, fil)


# Saving data
save(FinalData, file = "./Downloads/simulation_datasets.RData")

# Load data
# load("./Downloads/testing.RData")
...
##################################################################
########## 3. Multivariate T-Distribution Mixture Model ##########
##################################################################
...
print("K-means Results Processing....")
for (i in 1:80) {
...
print("Fuzzy C-Means Results Processing....")
for (i in 1:80) {
...
# Results from PD Clustering Initialization
...
print("Random Init Results Processing....")
for (i in 1:80) {
...
p1 = boxplot(df1, main="Data: Overlap 10% and Outlier 5% (n=300)",
        ylab= "Adjusted Rand Index", xlab="Initialization Algorithms",
        col = rainbow(5) )

p2 = boxplot(df2, main="Data: Overlap 10% and Outlier 15% (n=300)",
        ylab= "Adjusted Rand Index", xlab="Initialization Algorithms",
        col = rainbow(5) )

p3 = boxplot(df3, main="Data: Overlap 40% and Outlier 5% (n=300)",
        ylab= "Adjusted Rand Index", xlab="Initialization Algorithms",
        col = rainbow(5) )

p4 = boxplot(df4, main="Data: Overlap 40% and Outlier 15% (n=300)",
        ylab= "Adjusted Rand Index", xlab="Initialization Algorithms",
        col = rainbow(5) )

# Summary statistics for scenario 1
Summary<-p1$stats
colnames(Summary)<-p1$names
rownames(Summary)<-c("Min","First Quartile","Median","Third Quartile","Maximum")
Summary

# Summary statistics for scenario 2
Summary2<-p2$stats
colnames(Summary2)<-p2$names
rownames(Summary2)<-c("Min","First Quartile","Median","Third Quartile","Maximum")
Summary2

# Summary statistics for scenario 3
Summary3<-p3$stats
colnames(Summary3)<-p3$names
rownames(Summary3)<-c("Min","First Quartile","Median","Third Quartile","Maximum")
Summary3

# Summary statistics for scenario 4
Summary4<-p4$stats
colnames(Summary4)<-p4$names
rownames(Summary4)<-c("Min","First Quartile","Median","Third Quartile","Maximum")
Summary4

# Find average ARI for each scenario
# Scenario 1
colSums(df1)/length(df1[,1])
# Scenario 2
colSums(df2)/length(df2[,1])
# Scenario 3
colSums(df3)/length(df3[,1])
# Scenario 4
colSums(df4)/length(df4[,1])
